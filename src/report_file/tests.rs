...
fn writing_a_report_creates_the_file_with_the_document() {
    ...
    assert_eq!(write(&path, "{\"verdict\":\"ok\"}"), Ok(()));
    assert_eq!(
...
fn an_existing_report_is_replaced() {
    ...
    write(&path, "{\"verdict\":\"breaking\"}").expect("the directory exists");
    write(&path, "{}").expect("the directory exists");
...
fn a_missing_directory_is_rejected_before_the_analysis() {
    ...
    assert_eq!(
        check_dir(&missing.join("report.json")),
        Err(format!(
            "report directory '{}' does not exist",
            missing.display()
        ))
    );
    assert_eq!(check_dir(&dir.join("report.json")), Ok(()));
...
fn a_bare_file_name_writes_into_the_current_directory() {
    assert_eq!(check_dir(Path::new("report.json")), Ok(()));
}
